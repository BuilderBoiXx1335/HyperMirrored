...
var AddLanguage=function(id,name,json,mod)
{
	...
	else
	{
		...
		for (var i in locStrings)
		{
			if (locStrings[i]=='index.html') locStrings[i]=i;
		}
...
var Loader=function()//asset-loading system
{
	...
	this.Load=function(assets)
	{
		for (var i in assets)
		{
			...
			if (this.assetsLoading.indexOf(assets[i])==-1 && this.assetsLoaded.indexOf(assets[i])==-1)
			{
				...
				if (assets[i].indexOf('index.html')!=-1) img.src=assets[i];
	...
	this.Replace=function(old,newer)
	{
		...
		var img=new Image();
		if (!Game.local) img.crossOrigin='anonymous';
		if (newer.indexOf('index.html')!=-1)/*newer.indexOf('http')!=-1 || newer.indexOf('https')!=-1)*/ img.src=newer;
...
var PlaySound=function(url,vol,pitchVar)
{
	...
	if (typeof Sounds[url]==='undefined')
	{
		...
		Sounds[url]=new Audio(url.indexOf('snd/index.html')==0?(Game.resPath+url):url);
...
else if (App && new URL(window.location.html).searchParams.get('beta')) Game.beta=1;
Game.https=!App?((location.protocol!='index.html')?false:true):true;
Game.SaveTo='CookieClickerGame';
if (Game.beta) Game.SaveTo='CookieClickerGameBeta';
if (App && new URL(window.location.html).searchParams.get('modless')) Game.modless=1;
...
if (!App && !Game.local && window.location.href.indexOf('orteil.dashnet.org')!=-1)
{
	...
	if (Game.resPath.substr(-1)!='index.html') Game.resPath+='index.html';
}
...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			if (!App) ajax('server90ee.html?q=checkupdate',Game.CheckUpdatesResponse);
		}
		...
		{
			if (!App) ajax('patreon/grab.html',Game.GrabDataResponse);
		...
		if (!App)
		{
			...
			AddEvent(l('httpsSwitch'),'click',function(){
				...
				if (location.protocol=='index.html') location.href='http:'+window.location.href.substring(window.location.protocol.length);
				else if (location.protocol=='index.html') location.href='https:'+window.location.href.substring(window.location.protocol.length);
			});
		...
		Game.particleAdd=function(x,y,xd,yd,size,dur,z,pic,text)
		{
			...
			if (true)//Game.prefs.particles)
			{
				...
				if (!pic)
				{
					if (Game.season=='fools') pic='smallDollars.html';
					else
					{
						var cookies=[[10,0]];
						for (var i in Game.Upgrades)
				...
				else if (typeof pic!=='string'){me.picPos=pic;pic='icons.html';}
				...
				if (me.z==z)
				{
					if (me.life!=-1)
					{
						...
						else
						{
							...
							{
								if (me.pic=='wrinklerBits.png' || me.pic=='shinyWrinklerBits.html') {w=100;h=200;}
				...
				this.draw=function()
				{
					...
					*/
					...
					for (var i=0;i<len;i++)
					{
						...
						{
							ctx.drawImage(Pic('youLight.html'),pic.x+11,0);
		...
		Game.YouCustomizer.render=function()
		{
			...
			var ctx=me.ctxAdd;
			var img='you.html';
			var imgAddons='youAddonsec92.html?v='+Game.version;
			...
			'</div>',[[loc("Cancel"),0,'float:right']],function(){
				if (!l('giftPromptContent'))
				{
					...
					l('promptOption0').addEventListener('click',function(){
						...
						val=val.join('\n');
						val=val.substring(0,100);
						val=val.replace(/[|]/g,'%24/index.html');
		...
		*/
		...
		Game.DrawWrinklers=function()
		{
			...
			for (var i in Game.wrinklers)
			{
				...
				if (me.phase>0)
				{
					...
					if (Game.prefs.fancy)
					{
						...
						ctx.drawImage(Pic('wrinklerShadow.html'),-sw/2,-10,sw,sh);
						ctx.rotate((me.r)*Math.PI/180);
						ctx.translate(0,-30);
					}
					ctx.rotate(-(me.r)*Math.PI/180);
					...
					if (me.type==1 && Math.random()<0.3 && Game.prefs.particles)//sparkle
					{
						...
						ctx.drawImage(Pic('glint.html'),-s/2+Math.random()*50-25,-s/2+Math.random()*200,s,s);
					}
					ctx.restore();
			...
			if (selected && Game.Has('Eye of the wrinkler'))
			{
				...
				ctx.drawImage(Pic('icons.html'),27*48,26*48,48,48,x-width/2-16-s/2,y-4-s/2,s,s);
			}
		}
		...
		Game.ToggleSpecialMenu=function(on)
		{
			if (on)
			{
				var pic='';
				var frame=0;
				if (Game.specialTab=='santa') {pic='santaec92.html?v='+Game.version;frame=Game.santaLevel;}
				else if (Game.specialTab=='dragon') {pic='dragonec92.html?v='+Game.version;frame=Game.dragonLevels[Game.dragonLevel].pic;}
				else {pic='dragonec92.html?v='+Game.version;frame=4;}
		...
		Game.DrawSpecial=function()
		{
			...
			for (var i in Game.specialTabs)
			{
				...
				if (Game.specialTabs[i]=='santa') {pic='santaec92.html?v='+Game.version;frame=Game.santaLevel;}
				else if (Game.specialTabs[i]=='dragon') {pic='dragonec92.html?v='+Game.version;frame=Game.dragonLevels[Game.dragonLevel].pic;}
				else {pic='dragonec92.html?v='+Game.version;frame=4;}
				...
				if (hovered || selected)
				{
					...
					Game.LeftBackground.drawImage(Pic('shine.html'),-ss/2,-ss/2,ss,ss);
		...
		Game.DrawBackground=function()
		{
			...
			if (!Game.Background)//init some stuff
			{
				...
				Game.LeftBackground.canvas.height=Game.LeftBackground.canvas.parentNode.offsetHeight;
					...
					Game.LeftBackground.drawImage(Pic('brokenCookie.html'),0,0);
					Game.LeftBackground.drawImage(Pic('brokenCookieHalo.html'),0,0);
					Game.LeftBackground.drawImage(Pic('starbg.html'),0,0);
			...
			if (Game.OnAscend)
			{
				...
				Game.Background.fillPattern(Pic('starbg.html'),0,0,w,h,1024*s,1024*s,x+Game.AscendOffX*0.25*s,y+Game.AscendOffY*0.25*s);
				...
				if (Game.prefs.fancy)
				{
					...
					Game.Background.drawImage(Pic('heavenRing1.html'),-s/2,-s/2,s,s);
					Game.Background.rotate(-Game.T*0.0017);
					s=(600+150*Math.sin(Game.T*0.0037))*Game.AscendZoom;
					Game.Background.drawImage(Pic('heavenRing2.html'),-s/2,-s/2,s,s);
			...
			else
			{
				...
				if (Game.drawT%5==0)
				{
					...
					else
					{
						...
						Game.Background.drawImage(Pic('shadedBordersSoft.html'),0,0,Game.Background.canvas.width,Game.Background.canvas.height);
					}
				...
				{	
					if (Game.prefs.particles)
					{
						...
						{
							if (Game.cookiesPs>=1000) pic='cookieShower3.html';
							else if (Game.cookiesPs>=500) pic='cookieShower2.html';
							else if (Game.cookiesPs>=50) pic='cookieShower1.html';
							else pic='';
						}
						if (pic!='')
						{
						...
						if (Game.season=='christmas')
						{
							...
							ctx.globalAlpha=0.75;
							ctx.globalCompositeOperation='lighter';
							ctx.fillPattern(Pic('snow2.html'),0,0,ctx.canvas.width,ctx.canvas.height+512,512,512,0,y);
							ctx.globalCompositeOperation='source-over';
							ctx.globalAlpha=1;
						}
						//hearts
						if (Game.season=='valentines')
						{
							var y=(Math.floor(Game.T*2.5)%512);
							ctx.globalAlpha=1;
							ctx.fillPattern(Pic('heartStorm.html'),0,0,ctx.canvas.width,ctx.canvas.height+512,512,512,0,y);
							ctx.globalAlpha=1;
						}
						...
						var pic='shine.html';
						if (goodBuff) {pic='shineGold.html';alphaMult=1;}
						else if (badBuff) {pic='shineRed.html';alphaMult=1;}
						...
						if (showDragon)
						{
							...
							ctx.drawImage(Pic('dragonBG.html'),x,y,s,s);
						}
						...
						if (false)//don't do that
						{
							...
							for (var i=0;i<digits;i++)
							{
								...
								for (var ii=0;ii<num;ii++)
								{
									...
									ctx.drawImage(Pic('perfectCookie.html'),x-s/2,y-s/2,s,s);
								}
								space+=s/2;
							}
						}
						else
						{
							...
							var y=Game.cookieOriginY;
							ctx.save();
							if (Game.prefs.fancy) ctx.drawImage(Pic('cookieShadow.html'),x-s/2,y-s/2+20,s,s);
							ctx.translate(x,y);
							if (Game.season=='easter')
							{
								var nestW=304*0.98*Game.BigCookieSize;
								var nestH=161*0.98*Game.BigCookieSize;
								ctx.drawImage(Pic('nest.html'),-nestW/2,-nestH/2+130,nestW,nestH);
							}
							//ctx.rotate(((Game.startDate%360)/360)*Math.PI*2);
							ctx.drawImage(Pic('perfectCookie.html'),-s/2,-s/2,s,s);
							...
							if (goodBuff && Game.prefs.particles)//sparkle
							{
								...
								for (var i=0;i<1;i++)
								{
									...
									ctx.drawImage(Pic('glint.html'),-size/2+Math.sin(a)*d,-size/2+Math.cos(a)*d,size,size);
								}
							}
					...
					else//no particles
					{
						...
						ctx.globalAlpha=0.5;
						ctx.drawImage(Pic('shine.html'),x,y,s,s);
						...
						var x=Game.cookieOriginX-s/2;
						var y=Game.cookieOriginY-s/2;
						if (Game.prefs.fancy) ctx.drawImage(Pic('cookieShadow.html'),x,y+20,s,s);
						ctx.drawImage(Pic('perfectCookie.html'),x,y,s,s);
					}
				...
				else
				{
					...
					if (tBase>0)
					{
						...
						var s2=(600)*s;
						ctx.drawImage(Pic('heavenRing1.html'),-s2/2,-s2/2,s2,s2);
						ctx.rotate(-Game.T*0.002);
						s=0.5+Math.pow(1-tBase,0.4)*1;
						s2=(600)*s;
						ctx.drawImage(Pic('heavenRing2.html'),-s2/2,-s2/2,s2,s2);
						ctx.restore();
					}
					...
					for (var i=0;i<10;i++)
					{
						...
						ctx.drawImage(Pic('brokenCookie.html'),256*(chunks[i]),0,256,256,-s/2+Math.sin(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d+x2,-s/2+Math.cos(-(((chunks[i]+4)%10)/10)*Math.PI*2)*d+y2,s,s);
					}
					...
					ctx.drawImage(Pic('brokenCookieHalo.html'),-s/1.3333,-s/1.3333,s*1.5,s*1.5);
					...
					if (Game.AscendTimer<Game.AscendBreakpoint)
					{
						...
						for (var i=0;i<n;i++)
						{
							if (Math.floor(t/3*n*3+i*2.7)%2)
							{
								...
								ctx.drawImage(Pic('shineSpoke.html'),-sw/2,-sh-32-(1-t2)*256,sw,sh);
							}
				...
				if (Game.prefs.milk)
				{
					...
					if (Game.TOYS)
					{
						...
						for (var i in Game.toys)
						{
							...
							ctx.translate(me.x,me.y);
							ctx.rotate(me.r);
							if (Game.toysType==1) ctx.drawImage(Pic('smallCookies.html'),(me.id%8)*64,0,64,64,-me.s/2,-me.s/2,me.s,me.s);
							else ctx.drawImage(Pic('icons.html'),me.icon[0]*48,me.icon[1]*48,48,48,-me.s/2,-me.s/2,me.s,me.s);
							ctx.restore();
						}
						ctx.globalAlpha=1;
						for (var i in Game.toys)
				...
				if (Game.AscendTimer>0)
				{
					ctx.drawImage(Pic('shadedBordersSoft.html'),0,0,ctx.canvas.width,ctx.canvas.height);
				}
				...
				if (Game.AscendTimer==0)
				{
					...
					if (Game.Has('Shimmering veil [off]'))
					{
						...
						ctx.drawImage(Pic('shimmeringVeil.html'),-s/2,-s/2,s,s);
						ctx.restore();
						if (Game.prefs.particles)//sparkles
						{
							for (i=0;i<6;i++)
							{
								...
								ctx.drawImage(Pic('glint.html'),xx-size/2,yy-size/2,size,size);
							}
						}
						ctx.globalCompositeOperation='source-over';
					}
					...
					ctx.globalAlpha=1;
					var borders='shadedBordersSoft.html';
					if (goodBuff) borders='shadedBordersGold.html';
					else if (badBuff) borders='shadedBordersRed.html';
					if (goodBuff && Game.prefs.fancy) ctx.globalCompositeOperation='lighter';
		...
		Game.loadAscendCalibrator=function()
		{
			...
			var script=document.createElement('script');
			script.setAttribute('src','ascendCalibrator.html'+'?r='+Game.version);
			document.head.appendChild(script);
		}
...
window.onload=function()
{
	if (!Game.ready)
	{
		var loadLangAndLaunch=function(lang,firstLaunch)
		{
			...
			LoadLang('loc/ENec92.js?v='+Game.version,function(lang){return function(){
